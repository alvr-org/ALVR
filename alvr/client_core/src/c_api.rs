...
pub extern "C" fn alvr_initialize_logging() {
    catch_panic(crate::init_logging);
}

#[no_mangle]
pub unsafe extern "C" fn alvr_path_string_to_id(path: *const c_char) -> u64 {
    catch_panic(|| alvr_common::hash_string(CStr::from_ptr(path).to_str().unwrap()))
}
...
pub unsafe extern "C" fn alvr_dbg_client_impl(message: *const c_char) {
    catch_panic(|| alvr_common::dbg_client_impl!("{}", CStr::from_ptr(message).to_str().unwrap()));
}

#[no_mangle]
#[cfg_attr(not(debug_assertions), expect(unused_variables))]
pub unsafe extern "C" fn alvr_dbg_decoder(message: *const c_char) {
    catch_panic(|| alvr_common::dbg_decoder!("{}", CStr::from_ptr(message).to_str().unwrap()));
}
...
pub extern "C" fn alvr_mdns_service(service_buffer: *mut c_char) -> u64 {
    catch_panic(|| string_to_c_str(service_buffer, alvr_sockets::MDNS_SERVICE_TYPE))
}

/// To make sure the value is correct, call after alvr_initialize()
#[no_mangle]
pub extern "C" fn alvr_hostname(hostname_buffer: *mut c_char) -> u64 {
    catch_panic(|| string_to_c_str(hostname_buffer, &storage::Config::load().hostname))
}

/// To make sure the value is correct, call after alvr_initialize()
#[no_mangle]
pub extern "C" fn alvr_protocol_id(protocol_buffer: *mut c_char) -> u64 {
    catch_panic(|| string_to_c_str(protocol_buffer, &storage::Config::load().protocol_id))
}
...
pub unsafe extern "C" fn alvr_initialize_android_context(
    ...
    catch_panic(|| ndk_context::initialize_android_context(java_vm, context));
}
...
pub extern "C" fn alvr_report_fatal_decoder_error(message: *const c_char) {
    catch_panic(|| {
        if let Some(context) = &*CLIENT_CORE_CONTEXT.lock() {
            context
                .report_fatal_decoder_error(unsafe { CStr::from_ptr(message).to_str().unwrap() });
        }
    });
}
...
pub extern "C" fn alvr_initialize_opengl() {
    catch_panic(|| GRAPHICS_CONTEXT.set(Some(Rc::new(GraphicsContext::new_gl()))));
}

#[no_mangle]
pub extern "C" fn alvr_destroy_opengl() {
    catch_panic(|| GRAPHICS_CONTEXT.set(None));
}
...
pub unsafe extern "C" fn alvr_start_stream_opengl(config: AlvrStreamConfig) {
    catch_panic(|| {
        let view_resolution =
            UVec2::new(config.view_resolution_width, config.view_resolution_height);
                ...
                renderer.render(
                    ...
                    [
                        StreamViewParams {
                            ...
                            reprojection_rotation: from_capi_quat(
    ...
    catch_panic(|| {
        let config = VideoDecoderConfig {
            ...
            config_buffer: unsafe {
                slice::from_raw_parts(config.config_buffer, config.config_buffer_size as usize)
                    .to_vec()
            },
        };
...
pub extern "C" fn alvr_destroy_decoder() {
    catch_panic(|| *DECODER_SOURCE.lock() = None);
}
...
pub extern "C" fn alvr_rotation_delta(source: AlvrQuat, destination: AlvrQuat) -> AlvrQuat {
    catch_panic(|| to_capi_quat(from_capi_quat(source).inverse() * from_capi_quat(destination)))
}
