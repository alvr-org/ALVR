...
const _PLATFORM_TO_NAME : [(Platform, &str);19] = [
...
impl Platform {
    ...
    pub const fn uses_gamepads_only(&self) -> bool {
        matches!(
            self,
            Platform::AppleHeadset | Platform::AndroidUnknown
        )
    }

    pub const fn manufacturer_name(&self) -> &str {
        if matches!(self, Platform::Quest1) {
            "Oculus"
        }
        else if self.is_quest() {
            "Meta"
        }
        else if self.is_apple() {
            "Apple"
        }
        else if self.is_pico() {
            "Pico"
        }
        else if self.is_vive() {
            "HTC"
        }
        else {
            "Unknown"
...
impl Display for Platform {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        for (p, name) in _PLATFORM_TO_NAME {
            if p == *self {
                return write!(f, "{}", name)
            }
        }
...
pub fn platform_display_to_enum(name: &String) -> Platform {
    for (p, iter_name) in _PLATFORM_TO_NAME {
        if name == iter_name {
            return p
        }
    }
