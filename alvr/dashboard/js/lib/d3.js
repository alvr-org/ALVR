!(function () {
    ...
    d3.transpose = function (matrix) {
        ...
        for (
            var i = -1, m = d3.min(matrix, d3_transposeLength), transpose = new Array(m);
            ++i < m;
    ...
    function d3_event_dragSuppress(node) {
        ...
        if (d3_event_dragSelect == null) {
            d3_event_dragSelect =
                "onselectstart" in node ? false : d3_vendorSymbol(node.style, "userSelect");
        }
        ...
        return function (specifier) {
            ...
            return function (value) {
                ...
                var length =
                    ...
                    padding =
                        length < width ? new Array((length = width - length + 1)).join(fill) : "";
                if (zcomma)
                    before = formatGroup(
                        padding + before,
    ...
    d3_time.dayOfYear = function (date) {
        ...
        return Math.floor(
            (date - year - (date.getTimezoneOffset() - year.getTimezoneOffset()) * 6e4) / 864e5
        );
    };
        ...
        function (day, i) {
            ...
            var interval = (d3_time[day] = d3_time_interval(
                ...
                function (date) {
                    ...
                    return (
                        Math.floor((d3_time.dayOfYear(date) + ((day + i) % 7)) / 7) - (day !== i)
                    );
                }
            ));
    ...
    function d3_locale_timeFormat(locale) {
        ...
        function d3_time_parseWeekdayAbbrev(date, string, i) {
            ...
            return n
                ? ((date.w = d3_time_dayAbbrevLookup.get(n[0].toLowerCase())), i + n[0].length)
                : -1;
        }
        ...
        function d3_time_parseMonthAbbrev(date, string, i) {
            ...
            return n
                ? ((date.m = d3_time_monthAbbrevLookup.get(n[0].toLowerCase())), i + n[0].length)
                : -1;
        }
        function d3_time_parseMonth(date, string, i) {
            ...
            return n
                ? ((date.m = d3_time_monthLookup.get(n[0].toLowerCase())), i + n[0].length)
                : -1;
        }
        ...
        function linePoint(λ, φ) {
            ...
            if (p0) {
                ...
                if (antimeridian ^ (s * λ_ < λi && λi < s * λ)) {
                    ...
                    ((λi = ((λi + 360) % 360) - 180), antimeridian ^ (s * λ_ < λi && λi < s * λ))
                ) {
                    var φi = -inflection[1] * d3_degrees;
                    if (φi < φ0) φ0 = φi;
                } else {
    ...
    d3.geo.path = function () {
        ...
        path.context = function (_) {
            ...
            contextStream =
                (context = _) == null ? new d3_geo_pathBuffer() : new d3_geo_pathContext(_);
    ...
    function d3_geo_circleInterpolate(radius, precision) {
        ...
        return function (from, to, direction, listener) {
            ...
            for (var point, t = from; direction > 0 ? t > to : t < to; t -= step) {
                listener.point(
                    (point = d3_geo_spherical([cr, -sr * Math.cos(t), -sr * Math.sin(t)]))[0],
                    point[1]
                );
            }
        };
    ...
    d3.geo.distance = function (a, b) {
        var Δλ = (b[0] - a[0]) * d3_radians,
            ...
            t;
        return Math.atan2(
            Math.sqrt((t = cosφ1 * sinΔλ) * t + (t = cosφ0 * sinφ1 - sinφ0 * cosφ1 * cosΔλ) * t),
    ...
    function d3_geo_lengthLineStart() {
        ...
        function nextPoint(λ, φ) {
            ...
            d3_geo_lengthSum += Math.atan2(
                Math.sqrt(
                    (t = cosφ * Math.sin(t)) * t + (t = cosφ0 * sinφ - sinφ0 * cosφ * cosΔλ) * t
                ),
    ...
    function d3_geom_voronoi(sites, bbox) {
        ...
        while (true) {
            ...
            if (
                site &&
                (!circle || site.y < circle.y || (site.y === circle.y && site.x < circle.x))
            ) {
        ...
        function pie(data) {
            var n = data.length,
                ...
                a = +(typeof startAngle === "function"
                    ...
                    : startAngle),
                da =
                    (typeof endAngle === "function" ? endAngle.apply(this, arguments) : endAngle) -
                    a,
                p = Math.min(
                    Math.abs(da) / n,
    ...
    d3.layout.pack = function () {
        ...
        function pack(d, i) {
            ...
            if (padding) {
                var dr =
                    (padding * (radius ? 1 : Math.max((2 * root.r) / w, (2 * root.r) / h))) / 2;
                d3_layout_hierarchyVisitAfter(root, function (d) {
                    d.r += dr;
                });
                d3_layout_hierarchyVisitAfter(root, d3_layout_packSiblings);
        ...
        function apportion(v, w, ancestor) {
            if (w) {
                var vip = v,
                    ...
                    shift;
                while (
                    ((vim = d3_layout_treeRight(vim)), (vip = d3_layout_treeLeft(vip)), vim && vip)
                ) {
    ...
    function d3_scale_linearFormatPrecision(type, range) {
        ...
        return type in d3_scale_linearFormatSignificant
            ? Math.abs(p - d3_scale_linearPrecision(Math.max(abs(range[0]), abs(range[1])))) +
                  +(type !== "e")
            : p - (type === "%") * 2;
    }
    ...
    function d3_scale_log(linear, base, positive, domain) {
        function log(x) {
            return (
                (positive ? Math.log(x < 0 ? 0 : x) : -Math.log(x > 0 ? 0 : -x)) / Math.log(base)
            );
        }
        function pow(x) {
    ...
    function d3_scale_ordinal(domain, ranger) {
        ...
        function scale(x) {
            return range[
                ((index.get(x) || (ranger.t === "range" ? index.set(x, domain.push(x)) : NaN)) -
                    1) %
                    range.length
            ];
        }
        ...
        scale.rangeRoundBands = function (x, padding, outerPadding) {
            ...
            range = steps(
                start + Math.round((stop - start - (domain.length - padding) * step) / 2),
                step
            );
            if (reverse) range.reverse();
    ...
    d3.svg.arc = function () {
        ...
        function arc() {
            ...
            if (
                da > ε &&
                (rc = Math.min(Math.abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments))) > 0.001
            ) {
                cr = (r0 < r1) ^ cw ? 0 : 1;
                var rc1 = rc,
                    rc0 = rc;
        ...
        function axis(g) {
            g.each(function () {
                ...
                var range = d3_scaleRange(scale1),
                    ...
                    pathUpdate =
                        (path.enter().append("path").attr("class", "domain"), d3.transition(path));
    ...
    function d3_time_scale(linear, methods, format) {
        ...
        scale.nice = function (interval, skip) {
            ...
            function skipped(date) {
                return (
                    !isNaN(date) && !interval.range(date, d3_time_scaleDate(+date + 1), skip).length
                );
            }
            return scale.domain(
                d3_scale_nice(
