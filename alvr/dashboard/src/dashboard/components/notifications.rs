...
const NOTIFICATION_TIPS: &[&str] = &[
    ...
    r#"If you started having crashes after changing some settings, reset ALVR by re-running "Run setup wizard" from "Installation" tab and resetting settings in wizard, or deleting "session.json" from ALVR folder."#,
    ...
    r#"If the visual controller/hand models does not match the physical controller, you can tweak the offset in "Headset"->"Controllers"->"Left controller position/rotation offset" (affects both controllers)."#,
    ...
    // Miscellaneous
    r#"If your headset does not appear in the devices list it might be in a different subnet. Try "Add device manually" with IP shown from inside device."#,
