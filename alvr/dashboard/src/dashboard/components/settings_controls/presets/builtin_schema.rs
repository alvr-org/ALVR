...
pub fn panel_type_schema() -> PresetSchemaNode {
    const HELP: &str = r"Adapts the encoding pipeline for the headset's panel type.
LCD: Includes color correction that is not needed on OLED headsets.
OLED: Prioritizes dark scenes during encoding to reduce visible blocking artifacts.
OLED HDR: OLED preset with BT.2020 HDR enabled, for use with HDR injection tools such as SpecialK.
";

    PresetSchemaNode::HigherOrderChoice(HigherOrderChoiceSchema {
        name: "panel_type".into(),
