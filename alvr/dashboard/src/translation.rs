...
impl TranslationBundle {
    pub fn new(
        ...
        let requested_language = code.parse::<LanguageIdentifier>().unwrap();

        let languages = serde_json::from_str::<BTreeMap<String, String>>(list_raw).unwrap();

        let mut available_languages = vec![];
        for key in languages.keys() {
            let lang_code = key.parse().unwrap();
        ...
        let prederred_locale = resolved_locales.first().unwrap();
        let fallback_locale = resolved_locales.last().unwrap();
