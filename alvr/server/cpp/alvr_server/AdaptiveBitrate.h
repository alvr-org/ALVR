...
class AdaptiveBitrate {
  public:
        ...
        if (m_enableAdaptiveBitrate) {
            ...
            if (latencyUs != 0) {               // check valid latency
                if (latencyUs > m_adaptiveBitrateTarget + m_adaptiveBitrateThreshold) {
                    ...
                    else if (m_bitrateSent >
                             m_bitrate * m_adaptiveBitrateLightLoadThreshold *
                                 (m_framesPrevious == 0 ? m_refreshRate : m_framesPrevious) /
                                 m_refreshRate)
                        m_bitrate +=
                            m_adaptiveBitrateUpRate; // increase bitrate if sent mbps is higher than
                                                     // set bitrate threshold (set bitrate * load
                                                     // threshold * valid framerate)
                }
            }
        }
    ...
    void ResetSecond() {
        ...
        if (m_adaptiveBitrateUseFrametime) {
            if (m_framesPrevious > 0) {
                m_adaptiveBitrateTarget =
                    US_IN_S / m_framesPrevious +
                    m_adaptiveBitrateTargetOffset; // fps to frametime (us) + offset
            }
    ...
    // mbit/s
    uint64_t m_bitrate = 30;
    uint64_t m_bitrateUpdated = 30;
