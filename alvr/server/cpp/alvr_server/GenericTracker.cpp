...
GenericTracker::GenericTracker(std::string name)
    : m_unObjectId(vr::k_unTrackedDeviceIndexInvalid), m_name("ALVR/tracker/" + name),
      m_serialNumber("ALVR Tracker (" + name + ")") {}

vr::DriverPose_t GenericTracker::GetPose() { return m_pose; }

vr::EVRInitError GenericTracker::Activate(vr::TrackedDeviceIndex_t unObjectId) {
    ...
    vr_properties->SetStringProperty(propertyContainer,
                                     vr::Prop_TrackingSystemName_String,
                                     "ALVRTrackerCustom"); //"lighthouse");
    vr_properties->SetStringProperty(
        propertyContainer, vr::Prop_ModelNumber_String, "Vive Tracker Pro MV");
    vr_properties->SetStringProperty(
        propertyContainer, vr::Prop_SerialNumber_String, GetSerialNumber()); // Changed
    vr_properties->SetStringProperty(
    ...
    vr_properties->SetStringProperty(
        ...
        "BL 0 VRC 1541800000 Radio 1518800000"); // Changed
    vr_properties->SetStringProperty(propertyContainer,
                                     vr::Prop_HardwareRevision_String,
                                     "product 128 rev 2.5.6 lot 2000/0/0 0"); // Changed
    vr_properties->SetStringProperty(
        propertyContainer, vr::Prop_ConnectedWirelessDongle_String, "D0000BE000"); // Changed
    ...
    vr_properties->SetFloatProperty(
        propertyContainer, vr::Prop_DeviceBatteryPercentage_Float, 1.f); // Always charged

    vr::HmdMatrix34_t l_transform = {
        {{-1.f, 0.f, 0.f, 0.f}, {0.f, 0.f, -1.f, 0.f}, {0.f, -1.f, 0.f, 0.f}}};
    vr_properties->SetProperty(propertyContainer,
    ...
    vr_properties->SetUint64Property(
        propertyContainer, vr::Prop_HardwareRevision_Uint64, 2214720000); // Changed
    vr_properties->SetUint64Property(
        propertyContainer, vr::Prop_FirmwareVersion_Uint64, 1541800000); // Changed
    vr_properties->SetUint64Property(
        propertyContainer, vr::Prop_FPGAVersion_Uint64, 512); // Changed
    vr_properties->SetUint64Property(
        propertyContainer, vr::Prop_VRCVersion_Uint64, 1514800000); // Changed
    vr_properties->SetUint64Property(
        propertyContainer, vr::Prop_RadioVersion_Uint64, 1518800000); // Changed
    vr_properties->SetUint64Property(
        propertyContainer,
        vr::Prop_DongleVersion_Uint64,
        8933539758); // Changed, based on vr::Prop_ConnectedWirelessDongle_String above
    vr_properties->SetBoolProperty(
    ...
    vr_properties->SetStringProperty(
        propertyContainer, vr::Prop_RegisteredDeviceType_String, GetName());
    vr_properties->SetStringProperty(propertyContainer,
...
void GenericTracker::UpdateMotion(uint64_t targetTimestampNs, FfiBodyTracker tracker) {
