...
OvrController::OvrController(uint64_t devicePath, float *poseTimeOffset)
    ...
    memset(&this->pose, 0, sizeof(this->pose));
    ...
    this->pose.qDriverFromHeadRotation = EulerAngleToQuaternion(rotation);
    ...
    vr::HmdVector3d_t offetRes =
        vrmath::quaternionRotateVector(this->pose.qDriverFromHeadRotation, offset, false);

    this->pose.vecDriverFromHeadTranslation[0] = offetRes.v[0];
    this->pose.vecDriverFromHeadTranslation[1] = offetRes.v[1];
    this->pose.vecDriverFromHeadTranslation[2] = offetRes.v[2];

    this->pose.qWorldFromDriverRotation = HmdQuaternion_Init(1, 0, 0, 0);

    this->pose.qRotation = HmdQuaternion_Init(1, 0, 0, 0);
...
vr::DriverPose_t OvrController::GetPose() { return this->pose; }

int OvrController::getControllerIndex() { return device_path == LEFT_HAND_PATH ? 0 : 1; }
...
bool OvrController::onPoseUpdate(int controllerIndex, const TrackingInfo &info) {
    ...
    if (info.controller[controllerIndex].flags &
        ...
        vr::HmdQuaternion_t boneFixer = info.controller[controllerIndex].flags &
                                            ...
                                            : HmdQuaternion_Init(0.5, 0.5, 0.5, 0.5);
        this->pose.qRotation = QuatMultiply(&rootBoneRot, &boneFixer);
        this->pose.vecPosition[0] = info.controller[controllerIndex].boneRootPosition.x;
        this->pose.vecPosition[1] = info.controller[controllerIndex].boneRootPosition.y;
        this->pose.vecPosition[2] = info.controller[controllerIndex].boneRootPosition.z;

        if (info.controller[controllerIndex].flags &
            TrackingInfo::Controller::FLAG_CONTROLLER_LEFTHAND) {
            double bonePosFixer[3] = {0.0, 0.05, -0.05};
            vr::HmdVector3d_t posFix =
                vrmath::quaternionRotateVector(this->pose.qRotation, bonePosFixer);
            this->pose.vecPosition[0] =
                info.controller[controllerIndex].boneRootPosition.x + posFix.v[0];
            this->pose.vecPosition[1] =
                info.controller[controllerIndex].boneRootPosition.y + posFix.v[1];
            this->pose.vecPosition[2] =
        ...
        this->pose.qRotation = HmdQuaternion_Init(
        ...
        this->pose.vecPosition[0] = info.controller[controllerIndex].position.x;
        this->pose.vecPosition[1] = info.controller[controllerIndex].position.y;
        this->pose.vecPosition[2] = info.controller[controllerIndex].position.z;
    }
    ...
    angVel.v[0] = this->pose.vecAngularVelocity[0];
    angVel.v[1] = this->pose.vecAngularVelocity[1];
    angVel.v[2] = this->pose.vecAngularVelocity[2];
    vr::HmdVector3d_t angVelRes =
        vrmath::quaternionRotateVector(this->pose.qRotation, angVel, true);
    this->pose.vecAngularVelocity[0] = angVelRes.v[0];
    this->pose.vecAngularVelocity[1] = angVelRes.v[1];
    this->pose.vecAngularVelocity[2] = angVelRes.v[2];

    /*
    vr::HmdVector3d_t vel;
    vel.v[0] = this->pose.vecVelocity[0];
    vel.v[1] = this->pose.vecVelocity[1];
    vel.v[2] = this->pose.vecVelocity[2];
    vr::HmdVector3d_t velRes = vrmath::quaternionRotateVector(this->pose.qRotation, vel, true);
    this->pose.vecVelocity[0] = velRes.v[0];
    this->pose.vecVelocity[1] = velRes.v[1];
    this->pose.vecVelocity[2] = velRes.v[2];
    */

    /*
    double rotation[3] = { 0.0, 0.0, 36 * M_PI / 180 };
    this->pose.qDriverFromHeadRotation = EulerAngleToQuaternion(rotation);
    this->pose.vecDriverFromHeadTranslation[1] = 0.031153;
    this->pose.vecDriverFromHeadTranslation[2] = -0.042878;



    //double r[3] = { 0, -0.031153 ,0.042878 };
    double r[3] = { 0, 0 ,-0.053 };
    double v1[3] = { this->pose.vecVelocity[0], this->pose.vecVelocity[1], this->pose.vecVelocity[2]
    }; double w[3] = { this->pose.vecAngularVelocity[0], this->pose.vecAngularVelocity[1],
    this->pose.vecAngularVelocity[2] };
    ...
    this->pose.vecVelocity[0] = this->pose.vecVelocity[0] + tmp[0];
    this->pose.vecVelocity[1] = this->pose.vecVelocity[1] + tmp[1];
    this->pose.vecVelocity[2] = this->pose.vecVelocity[2] + tmp[2];
    */

    this->pose.poseTimeOffset = *m_poseTimeOffset;

    auto &c = info.controller[controllerIndex];

    if (c.flags & TrackingInfo::Controller::FLAG_CONTROLLER_OCULUS_HAND) {
        // this->pose.poseTimeOffset = 0.;
        float rotThumb =
        ...
        // Currently only the index pinch seems to have a system gesture. Otherwise just make sure
        // the pinching fingers have high confidence.
        bool registerIndexPinch =
            (c.handFingerConfidences & alvrIndexConfidence_High) &&
            (c.handFingerConfidences & alvrThumbConfidence_High) &&
            !(c.inputStateStatus & alvrInputStateHandStatus_SystemGestureProcessing);
        bool registerMiddlePinch = (c.handFingerConfidences & alvrMiddleConfidence_High) &&
                                   (c.handFingerConfidences & alvrThumbConfidence_High);
        bool registerRingPinch = (c.handFingerConfidences & alvrRingConfidence_High) &&
                                 (c.handFingerConfidences & alvrThumbConfidence_High);
        bool registerPinkyPinch = (c.handFingerConfidences & alvrPinkyConfidence_High) &&
                                  (c.handFingerConfidences & alvrThumbConfidence_High);
        ...
        case 6: // Oculus Quest
            vr::VRDriverInput()->UpdateBooleanComponent(
                ...
                registerPinkyPinch &&
                    (c.inputStateStatus & alvrInputStateHandStatus_PinkyPinching) != 0,
                0.0);
            vr::VRDriverInput()->UpdateBooleanComponent(
            ...
            if (this->device_path == RIGHT_HAND_PATH) {
                vr::VRDriverInput()->UpdateBooleanComponent(
                    ...
                    registerRingPinch &&
                        (c.inputStateStatus & alvrInputStateHandStatus_RingPinching) != 0,
                    0.0);
                vr::VRDriverInput()->UpdateBooleanComponent(
                ...
                vr::VRDriverInput()->UpdateBooleanComponent(
                    ...
                    registerMiddlePinch &&
                        (c.inputStateStatus & alvrInputStateHandStatus_MiddlePinching) != 0,
                    0.0);
                vr::VRDriverInput()->UpdateBooleanComponent(
            ...
            vr::VRDriverInput()->UpdateBooleanComponent(
                m_handles[ALVR_INPUT_JOYSTICK_CLICK], rotThumb > 0.9f, 0.0);
            ...
            vr::VRDriverInput()->UpdateBooleanComponent(
                ...
                registerIndexPinch &&
                    (c.inputStateStatus & alvrInputStateHandStatus_IndexPinching) != 0,
                0.0);
            vr::VRDriverInput()->UpdateScalarComponent(
                m_handles[ALVR_INPUT_TRIGGER_VALUE],
                registerIndexPinch ? c.fingerPinchStrengths[alvrFingerPinch_Index] : 0.0,
                0.0);
            vr::VRDriverInput()->UpdateBooleanComponent(
                m_handles[ALVR_INPUT_TRIGGER_TOUCH],
                registerIndexPinch && (c.fingerPinchStrengths[alvrFingerPinch_Index] > 0.7f),
                0.0);
            break;
        case 1: // Oculus Rift no pinch
        case 7: // Oculus Quest no pinch
...
#define APPSIZE4(a, b, c)                                                                          \
    do {                                                                                           \
        ...
        // m_boneTransform[HSB_RingFinger1].position); COPY3(c.bonePositionsBase[alvrHandBone_Ring2],
        // m_boneTransform[HSB_RingFinger2].position); COPY3(c.bonePositionsBase[alvrHandBone_Ring3],
