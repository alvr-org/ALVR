	...
	, AMFTextureReceiver receiver) : m_receiver(receiver)
{
	...
	if (codec == ALVR_CODEC_H264)
	{
		...
		//No noticable different between PRESET_QUALITY and PRESET_SPEED but the latter has better latency when the GPU is under heavy load
		...
		//Does not seem to make a difference but turned this on anyway in case it does on other hardware
