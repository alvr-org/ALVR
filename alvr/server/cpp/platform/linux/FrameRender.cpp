...
void FrameRender::AddPipeline(RenderPipeline *pipeline)
{
    pipeline->Build();
...
FrameRender::Output FrameRender::CreateOutput(uint32_t width, uint32_t height)
{
    m_output.imageInfo = {};
...
void FrameRender::commandBufferBegin()
{
    VkCommandBufferBeginInfo commandBufferBegin = {};
    commandBufferBegin.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    VK_CHECK(vkBeginCommandBuffer(m_commandBuffer, &commandBufferBegin));
...
void FrameRender::commandBufferSubmit()
{
    VK_CHECK(vkEndCommandBuffer(m_commandBuffer));
...
void FrameRender::addStagingImage(uint32_t width, uint32_t height)
{
...
uint32_t FrameRender::memoryTypeIndex(VkMemoryPropertyFlags properties, uint32_t typeBits) const
{
...
// RenderPipeline
RenderPipeline::RenderPipeline(FrameRender *render)
    : r(render)
{
}
