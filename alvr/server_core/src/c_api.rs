...
pub unsafe extern "C" fn alvr_get_time_ns() -> u64 {
    catch_panic(|| Instant::now().elapsed().as_nanos() as u64)
}
...
pub unsafe extern "C" fn alvr_path_to_id(path_string: *const c_char) -> u64 {
    catch_panic(|| alvr_common::hash_string(CStr::from_ptr(path_string).to_str().unwrap()))
}

#[no_mangle]
pub unsafe extern "C" fn alvr_error(string_ptr: *const c_char) {
    catch_panic(|| alvr_common::show_e(CStr::from_ptr(string_ptr).to_string_lossy()));
}

pub unsafe fn log(level: log::Level, string_ptr: *const c_char) {
    catch_panic(|| log::log!(level, "{}", CStr::from_ptr(string_ptr).to_string_lossy()));
}

#[no_mangle]
pub unsafe extern "C" fn alvr_warn(string_ptr: *const c_char) {
    catch_panic(|| log(log::Level::Warn, string_ptr));
}

#[no_mangle]
pub unsafe extern "C" fn alvr_info(string_ptr: *const c_char) {
    catch_panic(|| log(log::Level::Info, string_ptr));
}
...
pub unsafe extern "C" fn alvr_dbg_encoder(string_ptr: *const c_char) {
    catch_panic(|| alvr_common::dbg_encoder!("{}", CStr::from_ptr(string_ptr).to_string_lossy()));
}
...
pub extern "C" fn alvr_get_settings_json(buffer: *mut c_char) -> u64 {
    catch_panic(|| string_to_c_str(buffer, &serde_json::to_string(&crate::settings()).unwrap()))
}
...
pub unsafe extern "C" fn alvr_initialize_logging(
    ...
    catch_panic(|| {
        let session_log_path = (!session_log_path.is_null()).then(|| {
            PathBuf::from_str(CStr::from_ptr(session_log_path).to_str().unwrap()).unwrap()
        });
...
pub unsafe extern "C" fn alvr_shutdown() {
    catch_panic(|| SERVER_CORE_CONTEXT.write().take());
}
