...
/// Cleanup all static Rust resources to prevent lazy initialization after DLL unload.
/// This is critical for preventing crashes when Steam or other processes access ALVR
/// functions after the driver unload.
pub fn cleanup_static_resources() {
    dbg_server_core!("Cleaning up static resources before driver unload");
    
    // Force initialization/access of LazyLock statics to ensure they are already
    // initialized and won't be lazily initialized after DLL unload
    let _ = SESSION_MANAGER.read();
}
...
impl Drop for ServerCoreContext {
    fn drop(&mut self) {
        ...
        *self.lifecycle_state.write() = LifecycleState::ShuttingDown;
        ...
        // Explicitly drop webserver runtime to ensure all Tokio threads are joined
        dbg_server_core!("Shutting down webserver runtime");
