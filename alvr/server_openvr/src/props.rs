...
#[no_mangle]
pub unsafe extern "C" fn get_device_openvr_props(
    device_type: DEVICE_DESCRIPTION_TYPE,
    prop_count: *mut c_int,
) -> *mut FfiOpenvrProperty {
    let mut prop_vec;
    prop_vec = match device_type {
        DEVICE_DESCRIPTION_TYPE_HMD => get_hmd_openvr_prop(),
        DEVICE_DESCRIPTION_TYPE_LEFT_HAND => get_hand_openvr_prop(true),
        DEVICE_DESCRIPTION_TYPE_RIGHT_HAND => get_hand_openvr_prop(false),
        _ => Vec::<FfiOpenvrProperty>::new(),
    };

    prop_vec.shrink_to_fit();
    assert!(prop_vec.len() == prop_vec.capacity());

    let len = prop_vec.len();
    let ptr = prop_vec.as_mut_ptr();

    mem::forget(prop_vec);
    ptr::write(prop_count, len as c_int);

    ptr
}

#[no_mangle]
pub unsafe extern "C" fn free_prop_array(prop_array: *mut FfiOpenvrProperty, prop_count: c_int) {
    let len = prop_count as usize;
...
fn get_hmd_openvr_prop() -> Vec<FfiOpenvrProperty> {
    ...
    let mut set_prop = |prop| {
        info!("Setting head OpenVR prop: {prop:?}");

        props.push(to_ffi_openvr_prop(prop));
    };
...
fn get_hand_openvr_prop(left_hand: bool) -> Vec<FfiOpenvrProperty> {
    use OpenvrProperty::*;

    let mut props: Vec<FfiOpenvrProperty> = Vec::new();
    let settings = alvr_server_core::settings();
        ...
        match config.emulation_mode {
            ...
            ControllersEmulationMode::ViveTracker => {
                ...
                set_prop(FirmwareManualUpdateURL(
                    "https://developer.valvesoftware.com/wiki/SteamVR/HowTo_Update_Firmware".into(),
                ));
