...
struct InProgressPacket {
    stream_id: u16,
    packet_index: u32,
    buffer: Vec<u8>, // it doesn't contain a prefix
    buffer_size: usize,
    cursor: usize,
}
...
impl MultiplexedSocketReader for MultiplexedTcpReader {
    ...
    fn recv(&mut self, stream_queues: &HashMap<u16, StreamRecvQueues>) -> ConResult {
        let in_progress_packet = if let Some(packet) = &mut self.in_progress_packet {
            ...
            let mut prefix_bytes = [0; PACKET_PREFIX_SIZE];

            loop {
                let count = crate::socket_peek(&mut self.inner, &mut prefix_bytes)?;
            ...
            // This clears out the prefix from the stream. Errors here are unexpected, the data has
            // been already peeked, bail the connection if they happen
            self.inner.read_exact(&mut prefix_bytes).to_con()?;
